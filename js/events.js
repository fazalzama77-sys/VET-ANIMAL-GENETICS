/*
 * Safe renderer for ICAR-IVRI Animal Genetics and Breeding department announcements.
...
(() => {
    ...
    const SEEN_KEY = 'vgen-event-announcements-seen';
    ...
    const normalizeConfig = () => {
        const source = window.IVRI_GENETICS_EVENTS_CONFIG;
        ...
        return {
            ...
            title: safeText(source.sectionTitle, 'Animal Genetics Announcements'),
    ...
    const buildCard = (event) => {
        const card = make('article', 'vgen-event-card');
        if (event.featured === true) card.classList.add('is-featured');

        const meta = make('div', 'vgen-event-meta');
        meta.append(make('span', 'vgen-event-category', safeText(event.category, 'Academic Event')));
        if (event.published !== true) meta.append(make('span', 'vgen-event-draft', 'DRAFT PREVIEW'));

        const title = make('h3', '', safeText(event.title));
        const date = make('p', 'vgen-event-date');
        date.textContent = `📅 ${formatDate(event.date)}`;

        card.append(meta, title, date);
        const speaker = safeText(event.speaker);
        if (speaker) card.append(make('p', 'vgen-event-speaker', speaker));
        const description = safeText(event.description);
        if (description) card.append(make('p', 'vgen-event-description', description));

        const actions = make('div', 'vgen-event-actions');
        const watch = makeLink(event.youtubeUrl, 'Watch Video', 'vgen-event-action vgen-event-youtube', 'fas fa-play');
        const register = makeLink(event.registrationUrl, 'View Details', 'vgen-event-action vgen-event-register', 'fas fa-arrow-up-right-from-square');
    ...
    const injectStructuredData = (events) => {
        ...
        const payload = published.map((event) => ({
            ...
            organizer: { '@type': 'Organization', name: 'ICAR-Indian Veterinary Research Institute', url: 'https://animalgenetics.ivri.res.in/' }
        }));
        const script = document.createElement('script');
        script.type = 'application/ld+json';
        script.dataset.vgenEvents = 'true';
    ...
    const showAnnouncement = (event) => {
        ...
        if (document.querySelector('.vgen-event-dialog')) return;

        const overlay = make('div', 'vgen-event-dialog');
        const dialog = make('section', 'vgen-event-dialog-card');
        dialog.setAttribute('role', 'dialog');
        dialog.setAttribute('aria-modal', 'true');
        dialog.setAttribute('aria-labelledby', 'vgen-event-dialog-title');

        const close = make('button', 'vgen-event-dialog-close');
        close.type = 'button';
        close.setAttribute('aria-label', 'Close announcement');
        close.innerHTML = '&times;';

        dialog.append(close, make('div', 'vgen-event-dialog-kicker', safeText(event.category, 'Upcoming Animal Genetics Event')));
        const title = make('h2', '', safeText(event.title));
        title.id = 'vgen-event-dialog-title';
        dialog.append(title, make('p', 'vgen-event-dialog-date', formatDate(event.date)));
        if (safeText(event.description)) dialog.append(make('p', 'vgen-event-dialog-description', safeText(event.description)));

        const actions = make('div', 'vgen-event-dialog-actions');
        const watch = makeLink(event.youtubeUrl, 'Open Video', 'vgen-event-action vgen-event-youtube', 'fas fa-play');
        const register = makeLink(event.registrationUrl, 'View details', 'vgen-event-action vgen-event-register', 'fas fa-arrow-up-right-from-square');
        if (watch) actions.append(watch);
        if (register) actions.append(register);

        const later = make('button', 'vgen-event-later', 'Not now');
    ...
    const mountEventsSection = () => {
        try {
            ...
            if (!anchor) return;
            if (document.getElementById('vgen-events')) return;

            const section = make('section', 'vgen-events-section');
            section.id = 'vgen-events';
            section.setAttribute('aria-label', config.title);

            const header = make('div', 'vgen-events-header');
            const headingWrap = make('div');
            const eyebrow = make('div', 'vgen-events-eyebrow', 'ACADEMIC ANNOUNCEMENTS');
            const title = make('h2', '', config.title);
            headingWrap.append(eyebrow, title);
            if (config.subtitle) headingWrap.append(make('p', 'vgen-events-sub', config.subtitle));
            header.append(headingWrap);

            const channel = makeLink(config.youtubeUrl, config.youtubeLabel, 'vgen-events-channel', 'fas fa-video');
            if (channel) header.append(channel);
            section.append(header);

            if (config.events.length) {
                const grid = make('div', 'vgen-events-grid');
                config.events.forEach((event) => grid.append(buildCard(event)));
                section.append(grid);
            } else {
                const empty = make('div', 'vgen-events-empty');
                const emptyText = make('div');
                emptyText.append(
                    make('strong', '', config.emptyMessage),
                    make('span', '', 'New seminars, webinars, and animal breeding workshops will be posted here.')
                );
            ...
            console.warn('Animal Genetics events section skipped:', error);
        }
    };
