# tools/expand_glossary.py
# Expands js/glossary.js with 150 new exam-specific terms for B.V.Sc & A.H. Animal Genetics & Breeding
...
GLOSSARY_PATH = os.path.join(BASE_DIR, "js", "glossary.js")
REPO_GLOSSARY_PATH = os.path.join(BASE_DIR, "repo", "js", "glossary.js")

# The 150 new terms categorized across the 6 syllabus categories
NEW_TERMS = {
    ...
    "phenotypic correlation": {
        "term": "Phenotypic Correlation (r_P)",
        "category": "Quantitative Genetics & Inheritance",
        "def": "The observable linear association between two measurable traits in an animal, combining both genetic and environmental correlations: r_P = h_X h_Y r_G + e_X e_Y r_E."
    },
    ...
    "mass selection": {
        ...
        "def": "Selection based exclusively on the individual animal's own phenotypic performance records; mathematically most effective for traits with high heritability."
    },
    "pedigree selection": {
        "term": "Pedigree Selection",
        "category": "Animal Breeding & Selection Systems",
        "def": "Selection of breeding candidates based on the performance records of ancestors (parents and grandparents) before the candidate expresses the trait."
    },
    "sib selection": {
        "term": "Sib Selection",
        "category": "Animal Breeding & Selection Systems",
        "def": "Selection of breeding animals based on the performance records of collateral relatives (full-sibs or half-sibs); indispensable for post-slaughter carcass traits."
    },
    "progeny testing": {
        "term": "Progeny Testing",
        "category": "Animal Breeding & Selection Systems",
        "def": "Evaluating the genetic merit (breeding value) of a sire based on the average phenotypic performance of a large group of his daughters."
    },
    "independent culling": {
        "term": "Independent Culling Levels (ICL)",
        "category": "Animal Breeding & Selection Systems",
        "def": "A multi-trait selection method establishing minimum threshold standards for each trait simultaneously; failure in any single trait results in culling."
    },
    "tandem selection": {
        "term": "Tandem Selection",
        "category": "Animal Breeding & Selection Systems",
        "def": "A multi-trait selection method where selection is practiced for one trait at a time until the goal is achieved before switching to a second trait."
    },
    "selection index": {
        "term": "Selection Index (Hazel & Lush)",
        "category": "Animal Breeding & Selection Systems",
        "def": "A linear score combining multiple phenotypic traits weighted by economic weights and genetic parameters (I = Σ b_i X_i); mathematically the most efficient multi-trait selection method."
    },
    "genomic selection": {
    ...
    "blup": {
        "term": "Best Linear Unbiased Prediction (BLUP)",
        "category": "Animal Breeding & Selection Systems",
        "def": "Henderson's mixed model statistical algorithm that simultaneously estimates fixed environmental effects and random genetic breeding values using pedigree relationships."
    },
    "linebreeding": {
        "term": "Linebreeding",
        "category": "Animal Breeding & Selection Systems",
    ...
    "reciprocal recurrent selection": {
        ...
        "category": "Animal Breeding & Selection Systems",
        "def": "A breeding system using two genetically diverse populations as mutual testers for each other, simultaneously exploiting additive and non-additive gene action."
    },
    ...
    "open nucleus breeding system": {
        ...
        "category": "Animal Breeding & Selection Systems",
        "def": "A tiered breeding structure allowing top-performing females from commercial herds to enter the nucleus, reducing inbreeding and accelerating genetic progress."
    },
...
def main():
    print("Reading existing glossary...")
    with open(GLOSSARY_PATH, "r", encoding="utf-8") as f:
        content = f.read()

    # Extract existing categories and terms via regex/eval
    # Better yet, let's parse using node or python
    print(f"Total new terms defined: {len(NEW_TERMS)}")
    assert len(NEW_TERMS) == 150, f"Expected 150 new terms, got {len(NEW_TERMS)}"

    # Check for collisions with existing terms
    # We will run a node script to merge safely and produce pristine JS code.
